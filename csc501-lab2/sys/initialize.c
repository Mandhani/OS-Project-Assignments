...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	//setting the first four page tables, i.e., 0,1,2,3
	for(i=0;i<4;i++) {
		int f_number;
		get_frm(&f_number);
		frm_tab[f_number].fr_status = FRM_MAPPED;
		frm_tab[f_number].fr_pid = NULLPROC;
		frm_tab[f_number].fr_type = FR_TBL;
		frm_tab[f_number].fr_refcnt = 256;
		pt_t *pt;
		pt = (FRAME0 + f_number)*NBPG;
		for(j=0;j<PTD_SIZE;j++) {
			pt->pt_pres = 1;
			pt->pt_mbz = 0;
			pt->pt_write = 1;
			pt->pt_global = 1;
			pt->pt_avail = 0;
			pt->pt_base = (FRAME0)*(f_number) + j;
			pt++;
		}
	}
	/*int f_number;
	get_frm(&f_number);
	proctab[NULLPROC].pdbr = (FRAME0 + f_number) * NBPG;//setting pagedirectory to first frame. i.e., 0 - Not anymore
	frm_tab[f_number].fr_status = FRM_MAPPED;
	frm_tab[f_number].fr_pid = NULLPROC;
	frm_tab[f_number].fr_type = FR_DIR;
	pd_t *pd;
	pd = (FRAME0 + f_number) * NBPG;
	for(i=0;i<1024;i++) {
		pd->pd_write = 1;
		pd->pd_mbz = 0;
		if(i<4) {
	...
	create_pd(NULLPROC); // Setting pd for null proc., 4
