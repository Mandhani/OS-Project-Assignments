...
void create_pd(pid) {
	//kprintf("create_pd: Starting for process %d\n",pid);
	int f_number,i;
	get_frm(&f_number);
	proctab[pid].pdbr = (f_number + FRAME0)*NBPG;
	frm_tab[f_number].fr_status = FRM_MAPPED;
	frm_tab[f_number].fr_pid = pid;
	frm_tab[f_number].fr_type = FR_DIR;
	pd_t *pd;
	pd = (f_number + FRAME0)*NBPG;
	...
	//kprintf("create_pd: Done for process %d\n",pid);
	return;
}
